...
async function verifyAccessToken(accessToken: string): Promise<GoogleTokenInfo | null> {
...
const requestsByUid = new Map<string, { count: number; resetAt: number }>();
  ...
  if (!current || current.resetAt <= now) {
    if (requestsByUid.size > 10_000) {
      for (const [key, entry] of requestsByUid) {
        if (entry.resetAt <= now) requestsByUid.delete(key);
      }
    }
