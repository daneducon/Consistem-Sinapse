...
async function readYouTube(url: URL, videoId: string): Promise<string> {
  ...
  if (metadataResult.status === "fulfilled" && metadataResult.value.ok) {
    const metadata = await metadataResult.value.json() as Record<string, unknown>;
...
export async function readUrl(value: unknown): Promise<{ url: string; content: string }> {
  ...
  if (videoId) return { url: url.href, content: await readYouTube(url, videoId) };
  ...
  if (!response.ok) throw new HttpError(422, `O leitor de URL recusou o conteúdo (${response.status}).`);
